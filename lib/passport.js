...
passport.use('local.signin1', new LocalStrategy({
    ...
    console.log("local.signin1 async (req, username, password, done)")
    const rows = await database.query("SELECT * FROM iStock_users WHERE username = ?", [username]);
...
passport.use('local.changepassword', new LocalStrategy({
    ...
    newPassword1.password = await encryptPassword(password);

    // const result = await database.query("UPDATE iStock_users SET password = ? where user_name =?",[newPassword1.password,newPassword.username]);
    const result = await database.query(
        "UPDATE iStock_users SET password = "
        + newPassword1.password +
        " where user_name =" + newPassword.username);

    newPassword.id = result.insertId;

    console.log(newPassword);

    return done(null, newPassword, req.flash('success', 'Password Updated: '));
